...
class Regression:
	...
	def show_model_predictions(self, y_train_predicted, y_test_predicted):
		...
		plt.plot(self.data_preparation_object.years_train, self.data_preparation_object.y_train, "bo:", label="TimeSeries Data")
		plt.plot(self.data_preparation_object.years_train, y_train_predicted,"b", label="Fitted Additive Model") # prediction
        
		plt.plot(self.data_preparation_object.years_test, self.data_preparation_object.y_test, "ro:", label='TrueFuture Data')
		plt.plot(self.data_preparation_object.years_test, y_test_predicted, "r", label="Forecasted Additive Model Data")# prediction  
        
		ci = 1.96 * numpy.std(y_test_predicted)/numpy.sqrt(len(y_test_predicted))
		plt.fill_between(self.data_preparation_object.years_test, (y_test_predicted-ci).flatten(), (y_test_predicted+ci).flatten(), color='grey', alpha=.5, label="95 Confidence interval")
